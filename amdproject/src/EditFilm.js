...
function EditFilm() {
  ...
  },[title, date, genre,  age, country,crew, role, main]);
  ...
  const handleSubmit = async (e) => {
      ...
      const data ={ 
          ...
          isMainFilm: location.state.detail.ismain, 
      };
    ...
    const response = await fetch("http://localhost:8000/updateFilmDetails",{
         method: 'POST',
  ...
  return (
      <div>
          ...
          <form onSubmit={ handleSubmit}>
              ...
              <select value={location.state.detail.personrelated[0[0]]} onChange={(e) => setCrew(location.state.detail.personrelated[0[0]])}>
              {items?.map((item) => (
            ...
            <select value={location.state.detail.personrelated[0[1]]} onChange={(e) => setCrew(location.state.detail.personrelated[0[1]])}>
            
                <option>Actor</option>
                <option>Director</option>
            </select><br></br>
            <select value={location.state.detail.personrelated[1[0]]} onChange={(e) => setCrew(location.state.detail.personrelated[1[0]])}>
              {items?.map((item) => (
                <option selected value={item.personname} >
                  {item.personname}
                </option>
            ...
            <select value={location.state.detail.personrelated[1[1]]} onChange={(e) => setCrew(location.state.detail.personrelated[1[1]])}>
            
                <option>Actor</option>
                <option>Director</option>
            </select><br></br>
            <select value={location.state.detail.personrelated[2[0]]} onChange={(e) => setCrew(location.state.detail.personrelated[2[0]])}>
              {items?.map((item) => (
                <option selected value={item.personname} >
                  {item.personname}
                </option>
            ...
            <select value={location.state.detail.personrelated[2[1]]} onChange={(e) => setCrew(location.state.detail.personrelated[2[1]])}>
              ...
              <label>Is main Film</label>
              <input type="checkbox" value={location.state.detail.ismain} onChange={(e) => setMain(location.state.detail.ismain)}  ></input>  <br></br>
