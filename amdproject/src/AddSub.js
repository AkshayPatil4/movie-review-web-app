...
function AddSub() {
  ...
  const handleSubmit = async (e) => {
    ...
    const response = await fetch("http://localhost:8000/addNewSubFilm",{
         method: 'POST',
  ...
  const [itemss, setItemss] = useState([]);
   ...
   const datafItemss = await dataf.json();
     ...
     console.log(datafItemss.filmsData);
   };

return (
  <div className='create'>
      <h1>Add Subordinate Film Film</h1>
      ...
      <form onSubmit={ handleSubmit}>
          ...
          <select value={subf} onChange={(e) => setSubf(e.target.value)}><br></br>
              {itemss?.map((item) => (
                <option value={item.filmid} >
                  {item.filmname}
                </option>
              ))}
            </select><br></br>
